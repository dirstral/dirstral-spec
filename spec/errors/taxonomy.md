...
| `MEDIA_NO_TEXT` | No | `dir2mcp_open_file` on a `replace`-mode multimodal media chunk (SPEC §8.1.7) that has no text representation; permanent (non-retryable) — the hit can still be cited |
