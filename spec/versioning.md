...
|------|------------------------|-------|
| `dir2mcp` (Go) | `0.21.x` (0.22 partial) | Reference implementation; audited against `internal/` 2026-06-30 (dirstral-spec#29). Implements substantially the **0.15–0.21** surface (with the minor gaps noted below): extractor functional-check + graceful degradation (0.15); dual-machine corpus/index (`index.backend` memory/disk/qdrant/pgvector; remote `source.kind` local/nfs/s3 with stable `rel_path`) + the media transcription/translation/subtitle/quality-gate/derivation-identity surface (0.16); media-clip citations (`dir2mcp_open_media_clip` over `avutil.ExtractSegment`) + diarization speaker-aligned chunk boundary (0.17/0.19); CorpusFS abstraction + off-by-default distributed embedding (coordinator/workers/broker) (0.21). 0.18 retrieval-time dedup (`dedup.retrieval`) shipped; 0.18 §7.9 ingest-time canonicalization is unbuilt — it is marked **Status: Planned** in SPEC.md §7.9, so it is not a conformance miss. **0.22 per-language retrieval filter shipped end-to-end** (`languages[]` on `search`/`ask`, BCP-47 primary-subtag matching), but §8.8 **on-by-default language auto-detection is not yet wired** — language lands only via operator pin (`configured`) or sidecar (`declared`), not auto-`detected` (dir2mcp#471) — so the row is **`0.22 partial`**. Minor known gaps: distinct `TRANSLATE_FAILED`/`OCR_FAILED` codes (0.16 §14.4; dir2mcp#473) and the transcript-level `meta_json.words` granularity flag (0.20 §8.6.9; dir2mcp#472). The 0.13/0.14 multimodal-embedding arc (`gemini-embedding-2`, §8.1.7) is implemented but default-off and gated on the model's GA (Public Preview). The spec is authoritative — discrepancies are filed as spec-gap issues; maintainers decide whether to correct the spec or the implementation. |
...
- Recording: `coverage.languages[]` (`{start_ms, end_ms, language, language_source, language_confidence?, route, covered}`, coalesced) and `coverage.refused[]` (`{start_ms, end_ms, reason}`, `language_uncovered | quality_gate`) on the transcript `meta_json`; the representation `language` is the largest covered language by duration; a segment span in another language records `language` in its `extra_json`.
...
- **td-004 deferred a default to a closed issue (#78).** §A still said the
  ...
  closed in July. §B.1 of the same document already ranks the markup row
  docling (T1), pandoc (T2), `raw_text` (T4) and never bypasses a
  higher-fidelity active engine, so the two sections gave opposite conformance
  answers for the same `extractor: auto` configuration: an implementer
  following §A could emit lower-fidelity representations and believe it
  conformed. §A now states the fidelity order and defers the rule to §B.1. The
  two §A bullets had a second copy of the same drift: each named only "the
