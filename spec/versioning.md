...
- §15.1.1 **`Span`** (client-facing) + `spec/tools/schemas/common.json`: additive `region` variant (`start_page`/`end_page`, optional `bbox`, optional `section`). Backward compatible — `lines|page|time|document` unchanged; clients MUST ignore unrecognized kinds/fields.
- §9.3 **citation rendering**: section-breadcrumb example for region spans.
