...
* `coverage`: **conditional**. It is REQUIRED on every transcript produced by a
  decode of **two or more** windows (§8.6.13), including a fully decoded one, and
  is absent otherwise: on a single-request decode, on a sidecar, and from an
  implementation that does not window. **Which part of the recording the
  transcript actually covers.** Its fields are `windows_attempted`, `windows_decoded`,
...
1. **Hosted vendor surface → empty (native kinds).** For a `kind` whose embed requests go to a single hosted vendor surface (native `gemini`, `cohere`), an effective `base_url` that is unset or equal to **that vendor's hosted endpoint** normalizes to the **empty string** `""`. The match is per-**kind** (not per-profile-name, unlike rule 2): a native kind has exactly one hosted surface, so any profile of that kind pointed at it reaches the same vector space. Where an implementation accepts more than one spelling of that surface (e.g. Gemini's native embed base and the OpenAI-compatible base it is derived from), every spelling normalizes to `""`.

   A **custom** `base_url` on such a profile does **not** normalize away. Provider profiles expose `base_url` generically and implementations honor it — the embedding request really is sent to the operator's proxy, gateway or self-hosted deployment — so endpoint A and endpoint B can serve different vector spaces and MUST NOT compare identity-equal. Collapsing them was the defect fixed here: it let a corpus silently keep vectors from one endpoint while embedding new ones at another, in direct contradiction of the "MUST refuse to mix vector spaces" rule above.
...
**`""` is a valid identity component.** The empty string is a first-class, legitimate value of the `base_url` component, not a sentinel for "unknown". Consequently an index built **before** this rule — which recorded no `base_url` — is treated as having `base_url == ""` and remains **valid** on reload against any provider whose normalized `base_url` is also `""` (all built-in/hosted-default deployments, per rules 1–2). Only a corpus whose embed endpoint is a **non-canonical / custom** `base_url` sees a one-time `CONFIG_INVALID`/reindex on first reload after this change — the correct, bounded safety action, since those are exactly the corpora previously at risk of silent cross-endpoint mixing. The same bounded action applies to the native-kind correction in rule 1: a hosted-default `gemini`/`cohere` corpus is unaffected (it recorded `""` and still normalizes to `""`), while a corpus embedded through a **custom** native endpoint sees one `CONFIG_INVALID`/reindex — again exactly the corpora whose recorded identity could not distinguish two vector spaces. An implementation MUST surface that as the normal identity-mismatch error, never as a silent re-embed.

**The model components are the EFFECTIVE models (normative).** `text_model`/`code_model` MUST be the concrete model ids the implementation actually sends on the wire for each axis, **not** the raw configuration fields. Provider profiles are permitted to leave a model unset and let the adapter apply its own default (§8.1.1); recording the unset field would name no model at all, which makes the per-axis model fence inert for exactly those profiles — a later change to an adapter default, or a distributed worker (8.7.3) built from a binary that defaults differently, would compare identity-equal while producing vectors from another model space. Recording the resolved model also makes "write the current default into the config explicitly" a no-op rather than a spurious reindex, since the identity does not change.

* **Migration for a recorded EMPTY model component.** An implementation MUST treat an empty recorded `text_model`/`code_model` as compatible with whatever concrete model the current identity names for that axis, provided every other component matches. Such a recording predates this rule; its vectors were produced by the adapter default of the day, so the empty value means "the effective default", not "a different model". This grace is **one-directional and legacy-only**: it applies to the recorded side only, never lets a recorded **concrete** model match a different concrete model, and cannot arise in new recordings — so every identity written under this edition enforces the model fence in full.
...
* **Partial-transcript floor.** The decoded fraction is
  ...
  * **`skip`** (strict): **do not persist** the partial transcript. Record the
    item as `status=skipped` with `skip_reason="transcript_partial"` (§15.6), so
...
* **All windows failed is still an error.** When **no** window decodes, the
  ...
  unchanged (§7.7, and §8.6.12 for the per-track scoping): it is a transcription
  failure recorded as `documents.status=error`, not a coverage record. The
  distinction is the point: a failure is retried on the next run, a `skip` is a
  decision the daemon already made.
...
* **Coverage survives the transcript cache.** An implementation that caches
  ...
  is the defect this section exists to prevent. The coverage record MUST be
  written before the cached text it describes is published, and a coverage it
  cannot write MUST leave the text uncached: a cached transcript with no coverage
  claims to be whole. A **single-request** decode of bytes an earlier windowed
  decode cached MUST clear that entry's coverage record rather than inherit it.
