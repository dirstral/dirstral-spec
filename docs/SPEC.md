...
* When a paid route is called without valid payment, server returns HTTP `402 Payment Required` with machine-readable payment requirements in `PAYMENT-REQUIRED`. The challenge MUST carry a single-use server-issued `nonce`, an explicit validity window (`validAfter`/`validUntil`, `maxTimeoutSeconds`), and a first-class `resource` field, and MUST be bound to exactly one `(resource, cost)` pair — a proof valid for one tool/price MUST NOT be valid for another. The challenge wire profile is `X402Version: 2`. Canonical field schema: `docs/x402-payment-adapter-spec.md`.
