...
* `coverage`: the **parent→child linkage** — the exact fine chunks this summary
  ...
  * `source_rep_id`: the `rep_id` of the **single** representation whose chunks this
    ...
    so expansion can never pull in chunks the summary never saw.
  * `range`: the fine-unit range within `source_rep_id`, **inclusive** on both
    bounds, one of:
    ...
    * `{ "kind": "ordinals", "start": <int>, "end": <int> }` — an inclusive chunk
      `ordinal` range (§5.3) of `source_rep_id` (text `section` summaries).
    * `{ "kind": "time", "start_ms": <int>, "end_ms": <int> }` — an inclusive time
      range over the representation's transcript segments / clips (media
      `section`/event summaries, §8.6.1).
  Expansion (§9.7) resolves a summary to the chunks of `source_rep_id` whose
  `ordinal` (or segment `time` span, §5.4) falls in `range` — a **deterministic
  key**, not a vector-space match — so every fine chunk maps to **exactly one**
...
change. Cost is **O(documents + sections)**, not O(chunks). Generation is
